...
| _(none)_ | Show available commands and help user pick |

If no argument, tell the user:

> `/sindri init` — Set up improvement loop (explore project, design evaluate, scaffold .sindri/)
> `/sindri loop` — Start continuous experiment loop
> `/sindri cycle` — Run one experiment cycle
